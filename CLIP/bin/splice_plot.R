...
df <- read.table('results-bam/EWS.filtered.reads.splice-dist.txt', header=F, sep='\t', comment.char='', stringsAsFactors=F)
colnames(df) <- c('read', 'class', 'dist')
df$sample<-'enriched'
df$repl<-0

df_density <- density(df[abs(df$dist) < max_dist,]$dist, from=-max_dist, to=max_dist, n=2*max_dist)$y
#df_density <- density(df$dist, from=-max_dist, to=max_dist, n=2*max_dist)$y
...
while (i <= sample_count) {
    cat(paste0('Reading file: results-bam/readlists/EWS.failed.reads.',i,'.txt.bed.splice-dist\n'))
    df1 <- read.table(paste0('results-bam/readlists/EWS.failed.reads.',i,'.txt.bed.splice-dist'), header=F, sep='\t', comment.char='', stringsAsFactors=F)
    colnames(df1) <- c('read', 'class', 'dist')
    df1$sample<-'bg'
    df1$repl<-i
    
    dd <- density(df1[abs(df1$dist) < max_dist,]$dist, from=-max_dist, to=max_dist, n=2*max_dist)
#    dd <- density(df1$dist, from=-max_dist, to=max_dist, n=2*max_dist)
...
df0$class <- factor(df0$class, levels=c('SPLICE'))
...
p <- ggplot(bgdist_df, aes(x=x, y=val, linetype=class)) + 
    ...
    labs(title="Distance to differentially spliced event", x="Distance (bp)", y="Density") + 
    theme_bw() + 
    theme(legend.position="bottom", legend.title=element_blank(), legend.key = element_blank())
p
ggsave("figures/splice-dist.pdf", plot=p, height=4, width=4)
